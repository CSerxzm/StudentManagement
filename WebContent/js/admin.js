...
function insert(object) {
    ...
    if (object == "user") {
        ...
        url = "/StudentManagement/AdminDao?action=insert_user&username=" + username + "&password=" + password + "&level=" +encodeURI(encodeURI(level));
    }
        ...
        url = "/StudentManagement/AdminDao?action=insert_department&dno=" + dno + "&dname=" + encodeURI(encodeURI(dname));
        console.log(url);
    }
    else if (object == "class") {
        var classes = document.getElementById("show_insert_class").getElementsByTagName("input");
        var clname = classes[0].value.toString();
        var dno = classes[1].value.toString();
        url = "/StudentManagement/AdminDao?action=insert_class&clname=" + encodeURI(encodeURI(clname)) + "&dno=" + dno;
    }
    else if (object == "student") {
        var student = document.getElementById("show_insert_student").getElementsByTagName("input");
        var sname = student[0].value.toString();
        var ssex = null;
        if (student[1].checked == true)
            ssex = student[1].value.toString();
        else
            ssex = student[2].value.toString();
        var sage = student[3].value;
        var clno = student[4].value.toString();
        url = "/StudentManagement/AdminDao?action=insert_student&sname=" + encodeURI(encodeURI(sname)) + "&ssex=" +encodeURI(encodeURI(ssex))+ "&sage=" + sage + "&clno=" + clno;
    }
    else if (object == "course") {
        var course = document.getElementById("show_insert_course").getElementsByTagName("input");
        var cname = course[0].value.toString();
        var cteacher = course[1].value.toString();
        var ccredit = course[2].value;
        var maxnumofentrolled = course[3].value;
        url = "/StudentManagement/AdminDao?action=insert_course&cname=" +encodeURI(encodeURI( cname)) + "&cteacher=" +encodeURI(encodeURI(cteacher))  + "&ccredit=" + ccredit+"&maxnumofentrolled="+maxnumofentrolled;
    }
    else if (object == "sc") {
        var sc = document.getElementById("show_insert_sc").getElementsByTagName("input");
        var sno = sc[0].value;
        var cno = sc[1].value;
        var taskgrade = sc[2].value;
        var expgrade = sc[3].value;
        var examgrade = sc[4].value;
        url = "/StudentManagement/AdminDao?action=insert_sc&sno=" + sno + "&cno=" + cno + "&taskgrade=" + taskgrade+ "&expgrade=" + expgrade+ "&examgrade=" + examgrade;
    }
    else {
...
function show_alter(object) {
    ...
    if (object == "user") {
        show = "<div id='alter_user'  class='d_form'>"
            ...
            + "<input type='radio' name='after_level' value='学生' checked='checked'><span>学生</span>"
        ...
        show = "<div id='alter_student'  class='d_form'>"
            + "<h3>请输入需要修改的学生信息</h3>"
            + "<p>修改的学号</p>"
            + "<input type='text' autofocus='autofocus' name='sno' placeholder='学号' required>"
            + "<p>修改属性</p>"
            ...
            + "<input type='text' name='after_clno' placeholder='所在班级编号'>"
        ...
        show = "<div id='show_alter_course'  class='d_form'>"
            ...
            + "<input type='number' name='after_maxnumofentrolled' value placeholder='容量'>"
        ...
        show = "<div id='alter_sc'  class='d_form'>"
            ...
            + "<input type='text' name='cno' value placeholder='课程号' required>"
            + "<p>修改后</p>"
	        + "<input type='number' name='taskgrade' value placeholder='作业成绩'  required>"
	        + "<input type='number' name='expgrade' value placeholder='实验成绩'  required>"
	        + "<input type='number' name='examgrade' value placeholder='考试成绩'  required>"
...
function show_insert(object){
	...
	var show = null;
	if(object=="user"){
	    show = "<div id='show_insert_user'  class='d_form'>"
	        + "<h3>请输入新增用户信息</h3>"
	        ...
	        + "<p>权限级别</p>"
	        + "<div class='radio'>"
	        + "<input type='radio' name='level' value='教师' checked='checked'><span>教师</span>"
	        + "<input type='radio' name='level' value='学生'><span>学生</span>"
	        + "</div>"
	        + "<input id='submit' onclick=insert('user') type='button' name='submit' value='插入'>"
	        + "</div>";		
	}
	else if(object=="department"){
	    show = "<div id='show_insert_department'  class='d_form'>"
	        + "<h3>请输入新增院系信息</h3>"
	        + "<input type='text' autofocus='autofocus' name='dno' value placeholder='院系编号' required>"
	    ...
	    show = "<div id='show_insert_class'  class='d_form'>"
	        + "<h3>请输入新增班级信息</h3>"
	        + "<input type='text' name='clname'  autofocus='autofocus' placeholder='班级名称'>"
	        + "<input type='text' name='dno' placeholder='所属院系编号'>"
	    ...
	    show = "<div id='show_insert_student'  class='d_form'>"
	        + "<h3>请输入新增学生信息</h3>"
	        + "<input type='text' autofocus='autofocus' name='sname' value placeholder='姓名'>"
	        + "<p>性别</p>"
	        + "<div class='radio'>"
...
function alter_user() {
    ...
    var url = "/StudentManagement/AdminDao?action=alter_user&username=" + username + "&after_username=" + after_username + "&after_password=" + after_password + "&after_level=" + encodeURI(encodeURI(after_level));
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//修改院系
function alter_deprtment(){
    var xmlhttp;
    if (window.XMLHttpRequest) {
        //  IE7+, Firefox, Chrome, Opera, Safari 浏览器执行代码
        xmlhttp = new XMLHttpRequest();
    ...
    var url = "/StudentManagement/AdminDao?action=alter_department&dno=" + dno + "&after_dno=" + after_dno + "&after_dname=" + encodeURI(encodeURI(after_dname));
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//修改班级
function alter_class() {
    ...
    var clno = all[0].value.toString();
    var after_clname = all[1].value.toString();
    var after_dno = all[2].value.toString();
    var url = "/StudentManagement/AdminDao?action=alter_class&clno=" + clno +  "&after_clname=" + encodeURI(encodeURI(after_clname)) + "&after_dno=" + after_dno;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//修改学生
function alter_student() {
    ...
    var sno = all[0].value.toString();
    var after_sname = all[1].value.toString();
    var after_ssex = null;
    if (all[2].checked == true)
        after_ssex = all[2].value.toString();
    else
        after_ssex = all[3].value.toString();
    var after_sage = all[4].value;
    var after_clno = all[5].value.toString();
    var url = "/StudentManagement/AdminDao?action=alter_student&sno=" + sno + "&after_sname=" + encodeURI(encodeURI(after_sname)) + "&after_ssex=" + encodeURI(encodeURI(after_ssex))+"&after_sage="+after_sage+"&after_clno="+after_clno;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//修改课程
function alter_course() {
    ...
    var cno = all[0].value.toString();
    var after_cname = all[1].value.toString();
    var after_cteacher = all[2].value.toString();
    var after_ccredit = all[3].value;
    var after_maxnumofentrolled = all[4].value;
    var url = "/StudentManagement/AdminDao?action=alter_course&cno=" + cno + "&after_cname=" + encodeURI(encodeURI(after_cname)) + "&after_cteacher=" + encodeURI(encodeURI(after_cteacher))+"&after_ccredit="+after_ccredit+"&after_maxnumofentrolled="+after_maxnumofentrolled;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
//修改成绩
function alter_sc() {
    ...
    var sc = document.getElementById("alter_sc").getElementsByTagName("input");
    var sno = sc[0].value;
    var cno = sc[1].value;
    var taskgrade = sc[2].value;
    var expgrade = sc[3].value;
    var examgrade = sc[4].value;
    url = "/StudentManagement/AdminDao?action=alter_sc&sno=" + sno + "&cno=" + cno + "&taskgrade=" + taskgrade+ "&expgrade=" + expgrade+ "&examgrade=" + examgrade;
    xmlhttp.open("GET", url, true);
    xmlhttp.send();
}
